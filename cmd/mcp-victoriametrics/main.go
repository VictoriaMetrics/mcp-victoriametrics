...
Try not to second guess information - if you don't know something or lack information, it's better to ask.
	...
	if c.HasMultipleInstances() {
		instructions += fmt.Sprintf("\nThis server has multiple environments: %s. Use the optional `env` argument on API tools to target one. If omitted, `%s` is used.\n", strings.Join(c.InstanceNames(), ", "), c.DefaultInstanceName())
	}
